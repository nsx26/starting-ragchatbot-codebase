...
**`package.json`** — Defines the project's Node dev tooling. Contains three npm scripts:
...
**`.prettierignore`** — Excludes `node_modules/`, `chroma_db/`, and lock files from formatting.

**`scripts/check-quality.sh`** — Shell script that runs `prettier --check` and exits with a non-zero code if any file is out of format. Intended for CI or pre-commit hooks.
...
Added a multi-selector transition block targeting layout and content elements (`body`, `.sidebar`, `.chat-messages`, `.message-content`, `#chatInput`, etc.) so `background-color`, `color`, and `border-color` all animate over 0.3 s when the theme changes.
